...
class Sender(BasicSender.BasicSender):
        ...
        while True:
            ...
            while True:
                ...
                if not response:
                    ...
                    self.timeout = self.RTT(time.time(), connectiontimer)
                    connectiontimer = time.time()
    ...
    def RTT(self, ack_received, packet_sent, alpha=0.5):
        """ Return the new estimated RTT time based on
        exponential weighted averages."""
        sampleRTT = ack_received - packet_sent
        return alpha * self.timeout + (1-alpha)*sampleRTT
